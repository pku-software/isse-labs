...
def test_data_split_returns_four_parts(feature_target_sample):
    ...
    #   - parts 是元组(tuple)
    #   - 元组有四个元素
    assert isinstance(parts, tuple), "parts must be a tuple"
    assert len(parts) == 4, "part must contains 4 elements"
    pass
...
def test_end_to_end_train_and_eval(feature_target_sample):
    ...
    assert isinstance(score, float), f"score must be float type, it is {type(score)} now"
