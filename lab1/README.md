...
1. 在 GitHub 上从个人 fork 的 `<学号>` 分支，向课程仓库 `pku-software/isse-labs` 的 `main` 分支发起 PR，PR 标题使用以下格式：
   ...
   [Lab1] 姓名-学号
   ```
2. 等待助教更新仓库 `main` 分支。助教会修改每位同学所在行的内容，因此个人分支和课程仓库 `main` 会对同一行产生不同修改，此时 PR 页面将会提示存在冲突。
3. 点击 **Resolve conflicts**。删除 GitHub 显示的冲突标记，将本人所在行整理为“学院 姓名 学号”，不要修改其他同学的内容。
4. 点击 **Mark as resolved**，确认所有冲突均已处理后，点击 **Commit merge**。GitHub 会自动更新原 PR，无需重新创建或再次推送。
